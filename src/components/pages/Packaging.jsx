...
const UNIDADES_DISPONIBLES = ["un", "kg", "gr", "lts", "ml", "pack", "caja", "rollo", "otras"];
...
const Packaging = () => {
  const { packaging, guardarPackaging, eliminarPackaging } = useMush();
  ...
  const handleSubmit = async (e) => {
    ...
    if (!nombreLimpio) {
      setErrorNombre("El nombre del packaging es obligatorio.");
      return;
    }
    ...
    const duplicado = (packaging || []).find(
      (item) => item.id !== form.id && normalizar(item.nombre || "") === canonicoNuevo
    );

    if (duplicado) {
      setErrorNombre(`Ya existe un packaging registrado como "${duplicado.nombre}".`);
      return;
    }
    ...
    if (!idFinal && nombreLimpio) {
      ...
      idFinal = `pack_${baseSlug}_${randomSuffix}`;
    }

    const itemPrevio = (packaging || []).find((p) => p.id === form.id) || {};

    const packagingAGuardar = {
      ...itemPrevio,
      id: idFinal || "",
      nombre: nombreLimpio,
    ...
    try {
      await guardarPackaging(packagingAGuardar);

      Swal.fire({
        ...swalConfig,
        title: "Packaging guardado",
        icon: "success",
        timer: 1400,
  ...
  const packagingFiltrado = (packaging || [])
    .filter((item) => {
  ...
  return (
    <div className="container py-4">
      ...
      <div className="mx-auto" style={{ maxWidth: "920px", width: "100%", paddingBottom: "75px" }}>
        {/* Encabezado con Botón Nuevo Packaging (sin signo +) */}
        <div className="d-flex flex-wrap justify-content-between align-items-center mb-3 gap-3">
          <h2 className="mush-display text-white mb-0">Packaging</h2>

        </div>

        {/* Tabla de Listado de Packaging con scroll interno y fila única estricta */}
        <div className="mush-card p-3 p-sm-4">
          <div className="d-flex justify-content-between align-items-center gap-3 mb-3">
            <h5 className="text-white mb-0 fw-bold">Packaging</h5>
              ...
              {(packaging || []).length > 0 && (
                <div style={{ width: "240px", maxWidth: "100%" }}>
                  <BuscadorFiltro
                    ...
                    placeholder="Buscar packaging..."
                  />
                </div>
              )}
              <BotonExcel
                titulo="Packaging"
                columnas={["Packaging","Unidad","Observaciones"]}
                filas={() => packagingFiltrado.map((item) => [item.nombre, item.unidad, item.observaciones])}
              />
          ...
          <div className="table-responsive mush-scroll-tabla" style={{ maxHeight: "calc(100vh - 280px)" }}>
            <table className="table mush-tabla align-middle mb-0 text-nowrap">
              ...
              <tbody>
                {packagingFiltrado.length === 0 ? (
                  <tr>
                    <td colSpan="4" className="text-center py-4 text-secondary">
                      No hay packaging cargado.
                    </td>
                  </tr>
                ) : (
                  packagingFiltrado.map((item) => (
                    <tr key={item.id}>
                      <td>
                        <strong
      ...
      {/* MODAL para Nuevo / Editar Packaging con validación bajo la caja */}
      {mostrarModal && (
        <div
            ...
            <div className="modal-content mush-card p-3 p-sm-4 rounded-4 shadow-lg border border-secondary border-opacity-25">
              <div className="d-flex justify-content-between align-items-center mb-3">
                <h5 className="text-white mb-0 fw-bold fs-6">
                  ...
                  {modoEdicion ? "Editar Packaging" : "Nuevo Packaging"}
                </h5>
                <button
                  type="button"
              ...
              <form onSubmit={handleSubmit} noValidate autoComplete="off">
                {/* Packaging y Unidad en la misma fila */}
                <div className="row g-2 mb-2">
                  <div className="col-8">
                    ...
                    <input
                      ...
                      placeholder="Nombre del packaging"
                      value={form.nombre}
                ...
                <div className="d-flex justify-content-end gap-2 pt-1">
                  ...
                  <button
                    ...
                    style={{ fontSize: "0.82rem" }}
                  >
                    {modoEdicion ? "Guardar Cambios" : "Guardar Packaging"}
                  </button>
                </div>
              </form>
            </div>
